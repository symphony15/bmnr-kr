...
function StatCard({ label, value, sub, badge, badgeType = 'green' }) {
  return (
    <div className="card flex flex-col gap-2">
      <p className="stat-label">{label}</p>
      <p className="stat-value">{value}</p>
      {sub && <p className="text-sm text-gray-500">{sub}</p>}
      {badge && <span className={`badge-${badgeType} w-fit`}>{badge}</span>}
    </div>
  )
}

function EthPriceCard({ eth }) {
  const { price, change24h, loading, error, lastUpdated } = eth
  const isPositive = (change24h ?? 0) >= 0
  const changeColor = isPositive ? 'text-green-400' : 'text-red-400'
  const changeSign = isPositive ? '+' : ''
  const liveValue = price
    ? (SNAPSHOT.ethHoldings * price / 1_000_000_000).toFixed(1)
    : null
  ...
  return (
    <div className="card flex flex-col gap-2 relative">
      ...
      {loading ? (
        ...
        <>
          <p className="stat-value text-gray-500">${SNAPSHOT.ethPriceUSD.toLocaleString()}</p>
          <p className="text-xs text-red-400">API 오류 — 공시값 표시 중</p>
        </>
      ) : (
        <>
          <p className="stat-value">${price.toLocaleString()}</p>
          <p className={`text-sm font-medium ${changeColor}`}>
            {changeSign}{change24h.toFixed(2)}% (24h)
          </p>
          {liveValue && <span className="badge-green w-fit">보유가치 ${liveValue}B</span>}
          {lastUpdated && (
...
function BmnrStockCard({ bmnr }) {
  const { price, changePct, marketState, loading, error } = bmnr
  ...
  return (
    <div className="card flex flex-col gap-2 relative">
      {!loading && !error && price && (
        <span className="absolute top-3 right-3 flex items-center gap-1">
          <span className={`w-1.5 h-1.5 rounded-full ${isMarketOpen ? 'bg-green-500 animate-pulse' : 'bg-gray-500'}`} />
          <span className="text-[10px] text-gray-600">{isMarketOpen ? 'OPEN' : 'CLOSED'}</span>
        </span>
      )}
      <p className="stat-label">BMNR 주가 (NYSE)</p>
      {loading ? (
        <>
          <Skeleton className="h-8 w-3/4" />
          <Skeleton className="h-4 w-1/2" />
        ...
        <>
          <p className="stat-value text-gray-500">${SNAPSHOT.stockPriceUSD}</p>
          <p className="text-xs text-yellow-400">
            {error ? 'Worker 미연결 — 공시값 표시' : '공시값'}
          </p>
          <p className="text-[10px] text-gray-700">wrangler.toml 배포 후 실시간 연동</p>
        </>
      ) : (
        <>
          <p className="stat-value">${price.toFixed(2)}</p>
          {changePct != null && (
            <p className={`text-sm font-medium ${changeColor}`}>
              {changeSign}{changePct.toFixed(2)}% (오늘)
            </p>
          )}
        </>
      )}
...
function MNavCard({ mNAV, liveEthPrice }) {
  ...
  return (
    <div className="card flex flex-col gap-2">
      ...
      <p className="text-[10px] text-gray-700">🟡 bmnr.rocks 기준 추정값</p>
    </div>
  )
}
    ...
    <div className="card">
      <div className="flex items-center justify-between mb-3">
        <div>
          ...
          <p className="text-xs text-gray-600 mt-0.5">전체 ETH 발행량 대비 보유 비율 (1.207억개 기준)</p>
        </div>
      ...
      <div className="mt-3 p-3 bg-surface-3 rounded-lg">
        <p className="text-xs text-gray-500">
          ...
          <span className="ml-2 text-gray-600">— 10개월 만에 84.2% 달성</span>
        </p>
      </div>
    </div>
  )
    ...
    <section>
      <div className="flex flex-wrap items-center gap-2 mb-4">
        ...
        <span className="text-xs text-gray-600 bg-surface-3 px-2 py-0.5 rounded">
          공시일: {SNAPSHOT.dataDate}
        </span>
        <span className="badge-yellow">NYSE 공시 기준</span>
        {!eth.loading && !eth.error && (
          <span className="badge-green">ETH 실시간</span>
        )}
      </div>

      {/* 상단 6칸 그리드 — 모바일 2열, 데스크탑 3열 */}
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 mb-3">
        <StatCard
          label="ETH 보유량"
          value={SNAPSHOT.ethHoldings.toLocaleString()}
          sub={`≈ $${(SNAPSHOT.ethHoldings * (eth.price ?? SNAPSHOT.ethPriceUSD) / 1_000_000_000).toFixed(1)}B`}
        ...
        <StatCard
          ...
          sub={`전량 스테이킹 시 $${(SNAPSHOT.annualStakingProjectedUSD / 1_000_000).toFixed(0)}M`}
          badge="카운터파티 없는 이자"
          badgeType="green"
        />
        <EthPriceCard eth={eth} />
        <BmnrStockCard bmnr={bmnr} />
        <MNavCard mNAV={SNAPSHOT.mNAV} liveEthPrice={eth.price} />
      </div>
      ...
      <div className="mt-3 flex flex-wrap gap-3 text-xs text-gray-500">
        ...
        <span className="ml-auto text-gray-600">ETH: CoinGecko · 보유량: NYSE 공시 · 30초 갱신</span>
      </div>
    </section>
  )
}
